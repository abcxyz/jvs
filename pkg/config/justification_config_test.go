	...
	cases := []struct {
	...
	}{
		{
			...
			envs: map[string]string{
				...
				"JVS_PLUGIN_ABS_DIR":           "/var/jvs/pluginsDir",
			...
			wantConfig: &JustificationConfig{
				...
				Issuer:             "example.com",
				PluginAbsDir:       "/var/jvs/pluginsDir",
	...
	cases := []struct {
		...
		},
		{
			...
			cfg: &JustificationConfig{
				...
				SignerCacheTimeout: 5 * time.Minute,
				Issuer:             "jvs.abcxyz.dev",
				PluginAbsDir:       "./pluginsDir",
				DefaultTTL:         15 * time.Minute,
				MaxTTL:             4 * time.Hour,
			},
			wantErr: "PluginAbsDir is not absolute path",
		},
		{
			name: "empty_project",
			cfg: &JustificationConfig{
